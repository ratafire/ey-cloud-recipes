...
cron_interval = 20 #If this is not set your data will NOT be indexed
    ...
    if node[:name] == utility_name
      run_for_app(appname) do |app_name, data|
        ...
        execute "sphinx config" do
          command "bundle exec rake #{rake_name}:configure"
          user node[:owner_name]
          environment({
        ...
        execute "#{flavor} index" do
          command "bundle exec rake #{rake_name}:index"
          user node[:owner_name]
          environment({
            'HOME' => "/home/#{node[:owner_name]}",
            'RAILS_ENV' => node[:environment][:framework_env]
        ...
        if cron_interval
          cron "sphinx index" do
            ...
            month   '*'
            weekday '*'
            command "cd /data/#{app_name}/current && RAILS_ENV=#{node[:environment][:framework_env]} bundle exec rake #{rake_name}:index"
            user node[:owner_name]
          end
        end
      end
